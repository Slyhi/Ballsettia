...
class MainMenu extends Menu {
    constructor(initialLoad?: 'initialLoad') {
        ...
        this.select.name<Sprite>('cosmetic').updateCallback = function() {
            this.angle = Math.sin(4*this.life.time + 4) * 2.5;
        };
        this.select.name('cosmetic').addModule(new Button({
            hoverTint: 0xBFBFBF,
            clickTint: 0x808080,
            onJustHovered: juiceButton(1),
            onClick: () => {
                global.game.playSound('click');
                this.select.name<Sprite>('cosmetic').effects.post.filters = [];
                global.game.menuSystem.loadMenu(() => new PaletteMenu('friend'));
            },
        }));
