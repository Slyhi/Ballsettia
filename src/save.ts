...
type AutoBallsData = {
    ...
    paletteSettings?: PaletteMenu.Settings;
}
/**
 * REMEMBER TO SET MERGE FUNCTIONALITY IN mergeAutoBallsDatas() !!!!
 */
...
function mergeAutoBallsDatas(data: AutoBallsData, into: AutoBallsData) {
    ...
    into.paletteSettings = data.paletteSettings;
}
...
// BALLSETTIA'S COSMETIC SETTING O_O
// PALETTE SETTING
function loadPaletteSettings() {
    let defaults = PaletteMenu.GET_DEFAULT_SETTINGS();
    let settings = loadDataObject('paletteSettings', defaults);
    _.defaults(settings, defaults);
    return settings;
}