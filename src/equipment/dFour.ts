...
namespace Equipments {
    export class DFour extends OrbitEquipment {
        ...
        getDesc() {
            ...
            return `On ${PLAY}, take [gold]<coin>${this.goldRequirement} unspent[/gold] to permanently permanently gain status from [r]1<sword>[/r], [g]1<heart>[/g], [gold]1<star>[/gold], or gain [gold]<coin>4[/gold] next round randomly`;
        }

        get goldRequirement() { return 3; }
        ...
        private static onPlay(equipment: DFour, source: Ball, world: World) {
            ...
            let dieFace: number = Math.ceil(Math.random()*4);
